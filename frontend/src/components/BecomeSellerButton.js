...
const BecomeSellerButton = ({ compact = false }) => {
  ...
  const checkApplicationStatus = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Không có đơn đăng ký nào - user có thể tạo đơn mới
  ...
  const handleBecomeSellerClick = () => {
    ...
    // Nếu đã đăng nhập, kiểm tra lại trạng thái và mở modal đăng ký seller
    checkApplicationStatus();
    setShowModal(true);
  };
  ...
  if (applicationStatus?.status === 'rejected') {
    return (
      <>
        ...
        >
          {compact ? '❌ Đăng ký lại' : '❌ Đăng ký lại tài khoản Seller'}
        </Button>

        <Modal show={showModal} onHide={() => setShowModal(false)} size="lg">
          <Modal.Header closeButton>
            <Modal.Title>🛍️ Đăng ký lại tài khoản Seller</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Alert variant="danger" className="mb-4">
              <h6>❌ Đơn đăng ký trước đó bị từ chối</h6>
              <p><strong>Lý do từ chối:</strong> {applicationStatus.rejectionReason || 'Không có lý do cụ thể'}</p>
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <div className="d-grid gap-2">
                <Button 
                  ...
                  size="lg"
                >
                  {loading ? 'Đang đăng ký...' : 'Gửi đơn đăng ký lại'}
                </Button>
              </div>
            </Form>
          </Modal.Body>
